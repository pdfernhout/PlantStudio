...
class PdSinglePointValue(TObject):
    def __init__(self):
        self.x = 0.0
        self.y = 0.0
...
class PdEditNoteCommand(PdCommand):
    ...
    def createWithPlantAndNewTStrings(self, aPlant, aNewStrings):
        ...
        self.newStrings = aNewStrings as delphi_compatability.TStringList
        return self
...
class PdNewCommand(PdCommand):
    ...
    def doCommand(self):
        PdCommand.doCommand(self)
        self.newPlant = uplant.PdPlant().create()
        try:
            ucursor.cursor_startWait()
            if (self.useWizardPlant) and (self.wizardPlant != None):
                self.wizardPlant.copyTo(self.newPlant)
            else:
...
class PdBreedFromParentsCommand(PdCommand):
    def __init__(self):
        ...
        self.row = 0
        self.oldGenerations = TList()
        self.newGeneration = PdGeneration()
        self.firstGeneration = PdGeneration()
    ...
    def createWithInfo(self, existingGenerations, aFirstParent, aSecondParent, aRow, aFractionOfMaxAge, aCreateFirstGeneration):
        ...
        self.oldGenerations = delphi_compatability.TList().Create()
        if existingGenerations.Count > 0:
            for i in range(0, existingGenerations.Count):
                self.oldGenerations.Add(ugener.PdGeneration(existingGenerations.Items[i]))
        return self
    ...
    def doCommand(self):
        ...
        try:
            ...
            self.newGeneration = ugener.PdGeneration().create()
...
class PdReplaceBreederPlant(PdCommand):
    ...
    def doCommand(self):
        ...
        self.newPlant = uplant.PdPlant().create()
        self.plantDraggedFrom.copyTo(self.newPlant)
