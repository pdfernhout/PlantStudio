...
class PdLeaf(PdPlantPart):
    def __init__(self):
        ...
        self.compoundLeafRandomSwayIndexes = [0] * (range(0, kNumCompoundLeafRandomSwayIndexes + 1) + 1)
    ...
    def countPointsAndTrianglesFor3DExportAndAddToTraverserTotals(self, traverser):
        ...
        if self.isSeedlingLeaf:
            if self.plant.pSeedlingLeaf.leafTdoParams.scaleAtFullSize > 0:
                ...
                traverser.total3DExportTrianglesIn3DObjects += self.plant.pSeedlingLeaf.leafTdoParams.object3D.triangles.Count
            ...
            if self.plant.pLeaf.leafTdoParams.scaleAtFullSize > 0:
                ...
                traverser.total3DExportTrianglesIn3DObjects += self.plant.pLeaf.leafTdoParams.object3D.triangles.Count * self.plant.pLeaf.compoundNumLeaflets
            ...
            if self.plant.pLeaf.stipuleTdoParams.scaleAtFullSize > 0:
                ...
                traverser.total3DExportTrianglesIn3DObjects += self.plant.pLeaf.stipuleTdoParams.object3D.triangles.Count
    ...
    def drawWithDirection(self, direction):
        ...
        try:
            ...
            if (direction == uplant.kDirectionRight):
                turtle.rotateX(128)
            len = self.plant.pLeaf.petioleLengthAtOptimalBiomass_mm * self.propFullSize
            if (self.isSeedlingLeaf):
                len = len / 2
            ...
            if self.isSeedlingLeaf:
                self.drawStemSegment(len, width, angle, 0, self.plant.pLeaf.petioleColor, self.plant.pLeaf.petioleTaperIndex, u3dexport.kExportPartPetiole, upart.kUseAmendment)
                ...
                if (self.plant.pLeaf.compoundNumLeaflets <= 1) or self.plant.turtle.drawOptions.simpleLeavesOnly:
                    ...
                    self.drawStemSegment(len, width, angle, 0, self.plant.pLeaf.petioleColor, upart.kDontTaper, u3dexport.kExportPartPetiole, upart.kUseAmendment)
    ...
    def drawStipule(self):
        ...
        if self.plant.pLeaf.stipuleTdoParams.repetitions > 1:
            ...
            if leftOverDegrees > 0:
                ...
                addition = 0
            carryOver = 0
            for i in range(1, self.plant.pLeaf.stipuleTdoParams.repetitions + 1):
    ...
    def DrawLeafOrLeaflet(self, aScale):
        ...
        rotateAngle = 0.0
        tdo = KfObject3D()
        useFaceColor = TColorRef()
        useBackfaceColor = TColorRef()
    ...
    def drawCompoundLeafPinnate(self):
        ...
        if self.plant.pLeaf.compoundNumLeaflets <= 0:
            return
        for i in range(self.plant.pLeaf.compoundNumLeaflets, 1 + 1):
    ...
    def drawCompoundLeafInternode(self, count):
        ...
        len = self.plant.pLeaf.petioleLengthAtOptimalBiomass_mm * self.propFullSize * self.plant.pLeaf.compoundRachisToPetioleRatio / 100.0
        width = self.plant.pLeaf.petioleWidthAtOptimalBiomass_mm * self.propFullSize
        # v1.6b3
        ...
        self.drawStemSegment(len, width, angleZ, angleY, self.plant.pLeaf.petioleColor, upart.kDontTaper, u3dexport.kExportPartPetiole, upart.kDontUseAmendment)
    
    # v1.6b3
    def bendAngleForCompoundLeaf(self, count):
        result = 0.0
        leafletNumberEffect = 0.0
        propFullSizeThisLeaflet = 0.0
        difference = 0.0
    ...
    def DrawCompoundLeafPetioletCount(self, scale, aCount):
        ...
        len = scale * self.plant.pLeaf.petioleLengthAtOptimalBiomass_mm * self.propFullSize
        ...
        self.drawStemSegment(len, width, 0, angle, self.plant.pLeaf.petioleColor, self.plant.pLeaf.petioleTaperIndex, u3dexport.kExportPartPetiole, upart.kDontUseAmendment)
