...
class BreedingAndTimeSeriesOptionsStructure:
    def __init__(self):
        ...
        self.percentMaxAge = 0
        self.plantsPerGeneration = 0
...
class TdoParamsStructure:
    def __init__(self):
        ...
        self.alternateFaceColor = TColorRef()
        self.alternateBackfaceColor = TColorRef()
...
class ParamsGeneral:
    def __init__(self):
        ...
        self.growthSCurve = SCurveStructure()
...
class ParamsLeaf:
    def __init__(self):
        ...
        self.sCurveParams = SCurveStructure()
        self.petioleColor = TColorRef()
...
class ParamsFlower:
    def __init__(self):
        self.tdoParams = [0] * (range(kBud, kSepals + 1) + 1)
...
class ParamsInflorescence:
    def __init__(self):
        ...
        self.stalkColor = TColorRef()
        self.pedicelColor = TColorRef()
...
# aspects->stop 
class PdPlant(PdStreamableObject):
    def __init__(self):
        self.name = ""
        self.age = 0
        ...
        self.pInflor = [0] * (range(0, 1 + 1) + 1)
        self.pFlower = [0] * (range(0, 1 + 1) + 1)
        ...
        self.drawPositionIfFixed = TPoint()
        self.normalBoundsRect_pixels = TRect()
        ...
        self.turtle = KfTurtle()
        self.firstPhytomer = TObject()
        ...
        self.noteLines = TStringList()
        self.amendments = TListCollection()
    ...
    def create(self):
        ...
        self.previewCache = ubitmap.PdBitmap().Create()
        self.initializeCache(self.previewCache)
        self.randomNumberGenerator = urandom.PdRandom().create()
        self.breedingGenerator = urandom.PdRandom().create()
        self.amendments = ucollect.TListCollection().Create()
        self.breedingGenerator.setSeed(self.breedingGenerator.randomSeedFromTime())
        self.noteLines = delphi_compatability.TStringList.create
    ...
    def initialize3DObjects(self):
        ...
        self.pSeedlingLeaf.leafTdoParams.object3D = utdo.KfObject3D().create()
        self.pLeaf.leafTdoParams.object3D = utdo.KfObject3D().create()
        self.pLeaf.stipuleTdoParams.object3D = utdo.KfObject3D().create()
        self.pInflor[kGenderFemale].bractTdoParams.object3D = utdo.KfObject3D().create()
        self.pInflor[kGenderMale].bractTdoParams.object3D = utdo.KfObject3D().create()
        for partType in range(0, kHighestFloralPartConstant + 1):
            self.pFlower[kGenderFemale].tdoParams[partType].object3D = utdo.KfObject3D().create()
            self.pFlower[kGenderMale].tdoParams[partType].object3D = utdo.KfObject3D().create()
        self.pAxillaryBud.tdoParams.object3D = utdo.KfObject3D().create()
        self.pFruit.tdoParams.object3D = utdo.KfObject3D().create()
        self.pRoot.tdoParams.object3D = utdo.KfObject3D().create()
    
    def destroy(self):
        self.previewCache.free
        self.previewCache = None
        self.randomNumberGenerator.free
        self.randomNumberGenerator = None
        self.breedingGenerator.free
        self.breedingGenerator = None
        self.amendments.free
    ...
    def initializeCache(self, cache):
    ...
    def setName(self, newName):
        self.name = UNRESOLVED.copy(newName, 1, kPlantNameLength)
    ...
    def draw(self):
        ...
        if self.turtle.drawingSurface.pane != None:
    ...
    def hangingObjectsMemoryUse_K(self):
        ...
        if self.noteLines != None:
            # note
            result = result + self.noteLines.instanceSize / 1024.0
        if self.amendments != None:
            # amendments
            result = result + self.amendments.instanceSize / 1024.0
            if self.amendments.Count > 0:
                for i in range(0, self.amendments.Count):
                    result = result + uamendmt.PdPlantDrawingAmendment(self.amendments.Items[i]).instanceSize / 1024.0
        return result
    ...
    def recalculateBounds(self, drawNow):
        oldBasePoint = TPoint()
        drawingRect = TRect()
        intersectRect_pixels = TRect()
        intersectResult = false
        showProgress = false
        bytesThisBitmapWillBe = 0L
    ...
    def drawIntoCache(self, cache, size, considerDomainScale, immediate):
        newScaleX = 0.0
        newScaleY = 0.0
        drawPosition = TPoint()
        changed = false
        ...
        if immediate:
            ubmpsupport.fillBitmap(cache, udomain.domain.options.backgroundColor)
        self.turtle = uturtle.KfTurtle.defaultStartUsing()
        try:
            # set starting values 
            self.turtle.drawingSurface.pane = cache.Canvas
    ...
    def drawPreviewIntoCache(self, size, considerDomainScale, immediate):
        try:
            self.drawingIntoPreviewCache = true
            self.drawIntoCache(self.previewCache, size, considerDomainScale, immediate)
            self.previewCacheUpToDate = true
        finally:
            self.drawingIntoPreviewCache = false
    ...
    def shrinkPreviewCache(self):
        ...
        if (self.previewCache.Width > 5) or (self.previewCache.Height > 5):
            ...
            self.previewCache.TransparentMode = delphi_compatability.TTransparentMode.tmFixed
    ...
    def getHint(self, point):
        ...
        if (umain.MainForm.cursorModeForDrawing == umain.kCursorModePosingSelect) and (umain.MainForm.focusedPlant() == self):
            ...
            if (plantPart != None) and (plantPart.__class__ is upart.PdPlantPart):
                result = (plantPart as upart.PdPlantPart).getFullName()
        return result
    ...
    def boundsRect_pixels(self):
        result = TRect()
        result = self.normalBoundsRect_pixels
        return result
    ...
    def saveToGlobal3DOutputFile(self, indexOfPlant, translate, rectWithAllPlants, outputType, aTurtle):
        ...
        if outputType == u3dexport.k3DS:
            for i in range(0, u3dexport.kExportPartLast + 1):
                ...
                (fileExportSurface as u3dexport.KfDrawingSurfaceFor3DS).writeMaterialDescription(i, self.colorForDXFPartType(i))
            if not udomain.domain.registered:
                (fileExportSurface as u3dexport.KfDrawingSurfaceFor3DS).writeMaterialDescription(-1, UNRESOLVED.rgb(100, 100, 100))
    ...
    def defaultAllParameters(self):
        ...
        if udomain.domain.parameterManager.parameters.Count > 0:
            for i in range(0, udomain.domain.parameterManager.parameters.Count):
    ...
    def writeToPlantFile(self, plantFile):
        ...
        if self.noteLines.Count > 0:
            self.writeLine(plantFile, kStartNoteString)
            for i in range(0, self.noteLines.Count):
                self.writeLine(plantFile, self.noteLines.Strings[i])
        ...
        if self.amendments.Count > 0:
            for i in range(0, self.amendments.Count):
                if self.writingToMemo:
                    uamendmt.PdPlantDrawingAmendment(self.amendments.Items[i]).writeToMemo(umain.MainForm.plantsAsTextMemo)
                else:
                    uamendmt.PdPlantDrawingAmendment(self.amendments.Items[i]).writeToTextFile(plantFile)
        #v2.0
    ...
    def readLineAndTdoFromPlantFile(self, aLine, plantFile):
        ...
        if UNRESOLVED.pos(uppercase(kStartNoteString), uppercase(aLine)) > 0:
            ...
            self.noteLines.Clear()
            while not UNRESOLVED.eof(plantFile):
                noteLine = usupport.tolerantReadln(plantFile, noteLine)
                if (UNRESOLVED.pos(uppercase(kEndNoteString), uppercase(noteLine)) > 0) and (self.noteLines.Count < 5000):
                    break
                else:
                    self.noteLines.Add(noteLine)
        ...
        if UNRESOLVED.pos(uamendmt.kStartAmendmentString, aLine) > 0:
            ...
            return
        if (UNRESOLVED.pos("[", aLine) <= 0) or (UNRESOLVED.pos("=", aLine) <= 0):
            if self.readingFromMemo:
    ...
    def useBreedingOptionsAndPlantsToSetParameters(self, options, firstPlant, secondPlant, tdos):
        ...
        secondFloat = 0.0
        newFloat = 0.0
        firstColor = TColorRef()
        secondColor = TColorRef()
        newColor = TColorRef()
        firstSCurve = SCurveStructure()
        secondSCurve = SCurveStructure()
        newSCurve = SCurveStructure()
        firstTdo = KfObject3D()
        secondTdo = KfObject3D()
        newTdo = KfObject3D()
        randomTdo = KfObject3D()
        sectionIndex = 0
        paramIndex = 0
        saveAge = 0
        weight = 0.0
        ...
        self.reset()
        if udomain.domain.sectionManager.sections.Count > 0:
            for sectionIndex in range(0, udomain.domain.sectionManager.sections.Count):
    ...
    def tdoRandomlyPickedFromCurrentLibrary(self, tdos):
        ...
        if tdos.Count <= 0:
            return result
        index = umath.intMax(0, umath.intMin(tdos.Count - 1, intround(self.breedingGenerator.zeroToOne() * tdos.Count)))
    ...
    def streamDataWithFiler(self, filer, cvir):
        ...
        if filer.isReading():
            ...
            if hasFirstPhytomer:
                self.firstPhytomer = None
                self.firstPhytomer = uintern.PdInternode().create()
                if self.firstPhytomer != None:
    ...
    def nextDay(self):
        traverser = PdTraverser()
        newTotalBiomass_pctMPB = 0.0
        changeInTotalBiomassTodayAsPercentOfMPB_pct = 0.0
        fractionReproBiomassToday_frn = 0.0
        newReproBiomass_pctMPB = 0.0
        newShootBiomass_pctMPB = 0.0
        fractionToMaturity_frn = 0.0
            ...
            try:
                if (self.firstPhytomer == None):
                    ...
                    if self.firstPhytomer == None:
                        ...
                        return
                    uintern.PdInternode(self.firstPhytomer).setAsFirstPhytomer()
    ...
    def allocateOrRemoveBiomassWithTraverser(self, traverserProxy):
        ...
        if self.firstPhytomer == None:
            return
        traverser = traverserProxy as utravers.PdTraverser
        if traverser == None:
            return
    ...
    def allocateOrRemoveParticularBiomass(self, biomassToAddOrRemove_pctMPB, undistributedBiomass_pctMPB, askingMode, tellingMode, traverserProxy):
        traverser = PdTraverser()
        totalDemandOrAvailableBiomass_pctMPB = 0.0
    ...
    def amendmentForPartID(self, partID):
        result = PdPlantDrawingAmendment()
        i = 0
        ...
        if self.amendments.Count <= 0:
            return result
        for i in range(0, self.amendments.Count):
            amendment = uamendmt.PdPlantDrawingAmendment(self.amendments.Items[i])
    ...
    def clearPointersToAllAmendments(self):
        self.amendments.clearPointersWithoutDeletingObjects()
    
    def restoreAmendmentPointersToList(self, aList):
        i = 0
        
        if (aList != None) and (aList.Count > 0):
            for i in range(0, aList.Count):
                self.addAmendment(uamendmt.PdPlantDrawingAmendment(aList.Items[i]))
    ...
    def getInfoForPlantPartAtPoint(self, point, partID, partType):
        ...
        partID = -1
        partType = ""
        part = self.findPlantPartAtPositionByTestingPolygons(Point) as upart.PdPlantPart
        if part == None:
...
class PdPlantUpdateEvent(TObject):
    def __init__(self):
        self.plant = PdPlant()
        self.fieldID = 0
        self.fieldIndex = 0
