...
class PdDomain:
        ...
        if i < 0:
            ...
            while i < len(self.iniLines):
                ...
                if (len(aLine) > 0) and (aLine[1] == "["):
                    break
                if key == trim(usupport.stringUpTo(self.iniLines[i], "=")):
    ...
    def getProfileInformationFromFile(self, fileName):
        ...
        ### PDF PORT TEMPORARILY COMMENT OUT: self.bitmapOptions.colorType = UNRESOLVED.TPixelFormat(StrToInt(self.stringForSectionAndKey(section, "Colors (screen/2/16/256/15-bit/16-bit/24-bit/32-bit)", "6")))
        ...
        ### PDF PORT TEMPORARILY COMMENTED OUT: self.nozzleOptions.colorType = UNRESOLVED.TPixelFormat(StrToInt(self.stringForSectionAndKey(section, "Nozzle colors (screen/2/16/256/15-bit/16-bit/24-bit/32-bit)", "6")))
        ...
        # 24 bit
        ### PODF PORT __ REMPORARILY COMMENT OUT: self.animationOptions.colorType = UNRESOLVED.TPixelFormat(StrToInt(self.stringForSectionAndKey(section, "Animation colors (screen/2/16/256/15-bit/16-bit/24-bit/32-bit)", "6")))
        # -------------------------------------------  registration
    ...
    def boundProfileInformation(self):
        ...
        ### PDF PORT TEMP COMMENT OUT: self.bitmapOptions.colorType = UNRESOLVED.TPixelFormat(umath.intMax(0, umath.intMin(kMaxColorOption, ord(self.bitmapOptions.colorType))))
